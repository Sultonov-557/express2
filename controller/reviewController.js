...
async function get(req, res, next) {
    try {
        ...
        const data = await db.query(`SAF review WHID=${id} `);
...
async function findAll(req, res, next) {
    try {
        ...
        const items = await db.queryAll("SAF review");
        const pagination = new Pagination(items.length, paginationLimit, page);
        const data = await db.queryAll(`SAF review LIM ${pagination.limit} OFF ${pagination.offset}`);
...
async function update(req, res, next) {
    try {
        let { comment,userID,stars,productID, } = req.body;
        ...
        await db.query(`UP review SET ? WH ID= ${id}`, values);
        res.send("done");
    } catch (e) {
        next(e.message);
    }
...
async function remove(req, res, next) {
    try {
        const { id } = req.params;
        await db.query(`DAF review WH ID='${id}'`);
        res.send("done");
    } catch (e) {
        next(e.message);
    }
